            ...
            <div class="p-4">
                <div class="row">
                    <div class="col-md-12 mb-3">
                        Showing: <b>{{ $lgas->firstItem() }} - {{ $lgas->lastItem() }} </b> of <b>  {{ $lgas->total() }}</b><a href="{{ route('lga.create') }}" class="btn btn-outline-dark btn-sm float-right">New LGA</a>
                    </div>
                </div>
                <!-- Row -->
                <div class="row">
                        ...
                        <table class="table table-striped table-hover table-checkable order-column valign-middle dataTable no-footer"
                            ...
                            @forelse($lgas as $lga)
                                <tr class="" role="row">
                                    <td>{{ $lga->name }}</td>
                                    <td>{{ $lga->code}}</td>
                                    <td>{{ $lga->active? 'Active':'Disabled'}}</td>
                                    <td>{{ $lga->state->name }}</td>
                                    <td>
                                        <a href="{{ route('lga.show', $lga->unid) }}" class="btn btn-outline-dark btn-sm"><i class="icon-line-awesome-eye"></i> Preview</a>
